""" Test for Quantum Teledata Application
This script tests the quantum teledata application by simulating a simple two-node network.
It verifies that the teledata process correctly recreates the original quantum state.
"""
import itertools
import numpy as np
import pytest
from sequence.topology.dqc_net_topo import DQCNetTopo
from sequence.app.teledata_app import TeledataApp
...
def single_trial(psi, seeds=None):
    """Run a single trial of teledata with the given quantum state psi.
    Args:       
        psi (np.ndarray): The quantum state to teleport, represented as a numpy array.
    Returns:     
        np.ndarray: The quantum state after teledata, as received by Bob.
    """
    ...
    # log_filename = 'test_teledata.log'
    # log.set_logger(__name__, tl, log_filename)
    # log.set_logger_level('DEBUG')
    # modules = ['generation', 'teledata_app', 'teledata_protocol']
    ...
    # 2) Attach the TeledataApp on both nodes
    A = TeledataApp(alice)
    B = TeledataApp(bob)
...
def test_teledata_recreates_state(psi, seeds):
    """Test teledata for 25 cases (5 inputs x 5 seed sets)."""
    out = single_trial(psi, seeds=seeds)

    assert verify_same_state_vector(out, psi), f"teledata state {out} != original {psi}"


"""
Dual-teledata test (3 nodes): Alice teleports to Bob and Charlie concurrently.

We prepare two different states on Alice's data memories:
- data[0] -> (to Bob)
- data[1] -> (to Charlie)

A single TeledataApp is attached to Alice and .start(...) is called twice.
Bob and Charlie each get their own TeledataApp to record results.
"""

def dual_trial(psi_b: np.ndarray, psi_c: np.ndarray, seeds=None):
    """Run a dual-teledata trial on a 3-node network (alice, bob, charlie).
    Args:
    ...
    # import sequence.utils.log as log
    # log_filename = 'tmp/test_teledata.log'
    # log.set_logger(__name__, tl, log_filename)
    # log.set_logger_level('INFO')
    # # modules = ['generation', 'teledata_app', 'teledata_protocol', 'network_manager', 'resource_manager']
    # modules = ['generation', 'teledata_app', 'teledata_protocol']
    ...
    # ONE TeledataApp on Alice; separate apps on receivers
    A = TeledataApp(alice)
    B = TeledataApp(bob)
    ...
    # Kick off two concurrent teledatas from Alice
    ...
    assert len(B.results) > 0, "No teledata result recorded at Bob."
    assert len(C.results) > 0, "No teledata result recorded at Charlie."
...
def test_dual_teledata_recreates_states(psi_b, psi_c, seeds):
...
# test_dual_teledata_recreates_states()
# test_teledata_recreates_state()